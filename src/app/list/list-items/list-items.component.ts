...
import { ResultsState } from 'src/app/store/interfaces/ResultsState';
import { LayoutState } from 'src/app/store/interfaces/LayoutState';
import { ResultsFilter } from 'src/app/store/interfaces/ResultsFilter';
...
export class ListItemsComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  resultsState$: Observable<ResultsState>;
  ...
  updateFilterLabels(filters: ResultsFilter) {
  ...
  onRestoreList() {
    this.store.dispatch(
      ResultsActions.filter({ filters: { favorite: false } })
    );
  }
  ...
  onToggleFav() {
    ...
    this.store.dispatch(
      ResultsActions.filter({ filters: { favorite: this.isFavSelected } })
    );
  }
}
